...
- **One canonical money source per account** (`cdr_feed` XOR `statement` XOR `qbo_feed`) — never double-count. Enforced in ONE place, `assertCanonicalSource` (`src/lib/queries.ts`), which every writer must call: the rule previously lived half in each of two functions and a bank feed plus a statement upload on the same account counted every overlapping dollar twice (they fingerprint differently, so no downstream dedup catches it). QBO is reader/reconciler; never auto-post what a feed already provides.
...
- **`wrangler dev` WORKS locally** — verified 2026-08-03 on macOS 26.2 (the old "deploy-only, macOS 12.6 can't run workerd" note was years stale and was the stated reason money-writing paths shipped untested; the same premise already cost a prod-only React crash). `npx wrangler dev --port 8788` binds D1/R2/KV/DO locally and serves the SPA. Local D1 is seeded (`wrangler d1 execute tax-agent-db --local --file=schema.sql`). Two gotchas: `CLERK_ISSUER` is set in `wrangler.toml`, so override it (`--var CLERK_ISSUER:`) to reach the `DEV_AUTH_BYPASS=1` path; and flags come from `wrangler.toml`, so add one with `--var FEATURES:...`. Still run typecheck + `npm test` before deploy — but "I couldn't run it" is no longer true.
