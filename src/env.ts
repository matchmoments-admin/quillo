...
export interface TaxAgentRpc {
  ...
  deleteDocument(userId: string, docId: string): Promise<{ deleted: boolean; income_removed: number; txns_removed: number }>;
  ...
  setDeductibility(userId: string, txnIds: string[], state: string, deductibleAmountCents?: number | null, businessUsePct?: number | null): Promise<{ updated: number }>;
  ...
  recordCgtEvent(userId: string, e: { cgt_asset_id: string; fy?: string | null; event_type?: string | null; event_date: string; proceeds_cents: number; cost_base_used_cents: number; units_disposed?: number | null; discount_eligible?: boolean | null; method?: string | null }): Promise<string>;
  ...
  ignoreBatch(userId: string, txnIds: string[]): Promise<{ batch_id: string; updated: number; failures: { txnId: string; error: string }[] }>;
